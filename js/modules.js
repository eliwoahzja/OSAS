...
// Standard evacuation benchmark: under 4 minutes total campus clearance to designated open assembly field.
...
function incidentForm(el) {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    btn.textContent = 'Saving…';
    try {
      // TODO(privacy): In compliance with RA 10173 (Data Privacy Act of 2012), student disciplinary details
      // and minor medical data must remain confidential in all exports and staff-shared logs.
      // FIXME: If multiple emergency contacts are registered for this student, send to primary guardian first,
      // then queue secondary guardian if acknowledgment is unconfirmed within 15 minutes.
...
export function generateRiskRationale(factors, score, level) {
  ...
  // Simple, memorable recall rule for each band
  let recall = '';
  let action = '';
  switch (level) {
    case 'Critical':
  ...
  // Identify specific contributing drivers in plain English
  const elevated = [];
...
function showRiskExplanationModal(r) {
  let closeModal = () => {};
  ...
  if (isQuantitative) {
    ...
    // Plain-Language "Why" & Simple Recall section
    ...
    const whyCard = h('div', { class: 'rounded-2xl p-4 bg-gray-50 border border-gray-100 space-y-3' },
      ...
      // Narrative of contributing drivers
    ...
    // Calculation Breakdown block in the requested format
    const breakdownText = [
    ...
    body.appendChild(h('div', { class: 'space-y-4' },
      ...
      // Plain-Language "Why" & Simple Recall for legacy evaluations
...
function riskForm(el) {
  ...
  // Additional details: mitigation, owner, review_date
