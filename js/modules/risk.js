...
export function computeRiskScore(factors) {
  ...
  const raw = (threat * vulnerability * (exploit_likelihood * exploit_impact) * asset_value) - security_controls;
  return Math.max(0, raw); // a score can never be negative
}
...
export function enrichRiskWithLocalDetails(r) {
  if (!r) return r;
  if (api.dataMode() !== 'mock') return r; // live rows come from the database only
