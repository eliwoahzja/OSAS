...
function composer(el) {
  ...
  const renderFields = async () => {
    ...
    if (isIncident) {
      ...
      const broadcastToggle = h('div', { class: 'sm:col-span-2 flex items-start gap-2.5 p-3 rounded-xl bg-gray-50 border border-gray-200' },
        h('input', {
          type: 'checkbox',
          id: 'notify-all-parents',
          class: 'mt-0.5 w-4 h-4 accent-red-600 rounded cursor-pointer',
          checked: f.notify_all_parents,
          onchange: (e) => { f.notify_all_parents = e.target.checked; },
        }),
        h('label', { for: 'notify-all-parents', class: 'text-xs text-gray-700 cursor-pointer select-none' },
          h('span', { class: 'font-bold block text-gray-900' }, 'Notify All Parents (Campus-Wide Alert)'),
          h('span', { class: 'text-gray-500 block text-[11px] mt-0.5' }, 'Leave unchecked to email only the selected student’s registered parent / guardian.')),
      );
      ...
      h('button', {
        ...
        onclick: async (e) => {
          ...
          try {
            ...
            if (res.ok) {
              toast(isIncident
                ? (f.notify_all_parents ? 'Incident alert broadcast emailed to all parents.' : `Incident alert emailed to ${f.student_name || 'student'}’s parent/guardian.`)
                : 'Notification successfully emailed to parents.');
            } else {
