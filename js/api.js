...
export async function listRows(table, filters = {}) {
  ...
  try {
    ...
    provider = 'api';
    if (table === 'risks') {
      const localRows = MOCK.risks || [];
      const remoteIds = new Set((rows || []).map((r) => String(r.id)));
      const remoteHazards = new Set((rows || []).map((r) => String(r.hazard || '').toLowerCase().trim()));
      const extraLocal = localRows.filter((lr) => !remoteIds.has(String(lr.id)) && !remoteHazards.has(String(lr.hazard || '').toLowerCase().trim()));
      return [...extraLocal, ...(rows || [])];
    }
    return rows || [];
  } catch (err) {
  ...
  try {
    ...
    if (table === 'risks') {
      try {
        ...
        provider = 'api';
        const saved = (rows && rows[0]) || corePayload;
        insertMock(table, { ...payload, id: saved.id || payload.id });
...
export async function sendNotification(rawPayload = {}) {
  ...
  if (isParentAlert) {
    if (rawPayload.related_incident_id && isValidUuid(rawPayload.related_incident_id)) {
      payload.related_incident_id = rawPayload.related_incident_id;
    }
    ...
    if (rawPayload.notify_all_parents != null) payload.notify_all_parents = Boolean(rawPayload.notify_all_parents);
  ...
  delete record.notify_all_parents;
