...
CREATE TABLE risks (
    ...
    risk_level VARCHAR(15) NOT NULL CHECK (risk_level IN ('Low', 'Medium', 'Moderate', 'High', 'Critical')),
    mitigation TEXT,
    owner VARCHAR(255),
    review_date DATE,
    threat SMALLINT CHECK (threat BETWEEN 1 AND 5),
    vulnerability SMALLINT CHECK (vulnerability BETWEEN 1 AND 5),
    exploit_likelihood SMALLINT CHECK (exploit_likelihood BETWEEN 1 AND 5),
    exploit_impact SMALLINT CHECK (exploit_impact BETWEEN 1 AND 5),
    asset_value SMALLINT CHECK (asset_value BETWEEN 1 AND 5),
    security_controls SMALLINT CHECK (security_controls BETWEEN 1 AND 5),
    risk_score INT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
